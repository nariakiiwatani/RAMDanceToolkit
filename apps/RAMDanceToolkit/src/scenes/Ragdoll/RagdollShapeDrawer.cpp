// 
// RagdollShapeDrawer.cpp - RAMDanceToolkit
// 
...
void RagdollShapeDrawer::drawCoordSystem()  {
	glBegin(GL_LINES);
	glColor3f(1, 0, 0);
	glVertex3d(0, 0, 0);
	glVertex3d(1, 0, 0);
...
void RagdollShapeDrawer::drawSphere(btScalar radius, int lats, int longs) 
{
	int i, j;
	for(i = 0; i <= lats; i++) {
...
void RagdollShapeDrawer::drawCylinder(float radius,float halfHeight, int upAxis)
{
...
RagdollShapeDrawer::ShapeCache*		RagdollShapeDrawer::cache(btConvexShape* shape)
{
...
void RagdollShapeDrawer::drawOpenGL(btScalar* m, const btCollisionShape* shape, const btVector3& color)
{
    drawOpenGL(m, shape, color, 0, btVector3(), btVector3());
}

void RagdollShapeDrawer::drawShadow(btScalar* m, const btVector3& extrusion, const btCollisionShape* shape)
{
    drawShadow(m, extrusion, shape, btVector3(), btVector3());
}

void RagdollShapeDrawer::drawOpenGL(btScalar* m, const btCollisionShape* shape, const btVector3& color,int	debugMode,const btVector3& worldBoundsMin,const btVector3& worldBoundsMax)
{
...
//
void		RagdollShapeDrawer::drawShadow(btScalar* m,const btVector3& extrusion,const btCollisionShape* shape,const btVector3& worldBoundsMin,const btVector3& worldBoundsMax)
{
	glPushMatrix(); 
	//btglMultMatrix(m);
    glMultMatrixf(m);
...
//
RagdollShapeDrawer::RagdollShapeDrawer()
{
	m_texturehandle			=	0;
